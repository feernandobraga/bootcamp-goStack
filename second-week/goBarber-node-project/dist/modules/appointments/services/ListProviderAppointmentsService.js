...
let ListProviderAppointmentsService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    let appointments = await this.cacheProvider.recover(cacheKey); // retrieve all appointments in redis for that given key

    console.log(`cache created ${cacheKey}`);
    console.log(`content from cache ${(_appointments = appointments) === null || _appointments === void 0 ? void 0 : _appointments.length}`); // appointments = null;
